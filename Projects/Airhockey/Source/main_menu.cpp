...
ICRESULT MainMenu::Init(icContentLoader* pContentLoader)
{
    ...
    // two player button
    m_btnTwoPlayer = new icButton(m_pContent);
    m_btnTwoPlayer->SetTextures(tex1,tex2);
    size.Set(512.0f*rat,157.0f*rat);
    pos.Set(mx,sy*0.709f);
    //col.Set(0,0,0,255);
    m_btnTwoPlayer->SetSize(size);
    m_btnTwoPlayer->SetPos(pos);
    m_btnTwoPlayer->SetColor(col);
    tl.Set(0.0f,0.807f);
    tr.Set(1.0f,0.807f);
    bl.Set(0.0f,0.669f);
    br.Set(1.0f,0.669f);
    m_btnTwoPlayer->SetUVs(tl,tr,bl,br);
    m_btnTwoPlayer->SetHighUVs(tl,tr,bl,br);
    m_btnTwoPlayer->SetSounds(m_pClick,m_pHover);
    m_uiFrame->AddChild(m_btnTwoPlayer);

    // network play button
    m_btnNetPlay = new icButton(m_pContent);
    m_btnNetPlay->SetTextures(tex1,tex2);
    size.Set(512.0f*rat,248.0f*rat);
    pos.Set(mx,sy*0.5f);
    //col.Set(0,0,0,255);
    m_btnNetPlay->SetSize(size);
    m_btnNetPlay->SetPos(pos);
    m_btnNetPlay->SetColor(col);
    tl.Set(0.0f,0.669f);
    tr.Set(1.0f,0.669f);
    bl.Set(0.0f,0.385f);
    br.Set(1.0f,0.385f);
    m_btnNetPlay->SetUVs(tl,tr,bl,br);
    m_btnNetPlay->SetHighUVs(tl,tr,bl,br);
    m_btnNetPlay->SetSounds(m_pClick,m_pHover);
    m_uiFrame->AddChild(m_btnNetPlay);
...
ICRESULT MainMenu::Update(const float fDeltaTime, bool* bFinished)
{
    ...
    if (m_btnOnePlayer->WasPressed())
    {
        ...
        gApp.m_GameState.GameMode = GAME_MODE_SINGLE;
        gApp.GetStateMachine().PushState(&gApp.m_GameState);
    }
    else if (m_btnTwoPlayer->WasPressed())
    {
        m_btnTwoPlayer->SetDown(false);
        m_btnTwoPlayer->SetDown(false);

        gApp.m_GameState.GameMode = GAME_MODE_TWO;
        gApp.GetStateMachine().PushState(&gApp.m_GameState);
    }
    else if (m_btnNetPlay->WasPressed())
    {
        m_bRenderTopOnly = false;
        gApp.GetStateMachine().PushState(&gApp.m_NetMenu);
    }
